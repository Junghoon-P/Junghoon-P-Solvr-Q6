...
export const SleepAnalysis: React.FC<SleepAnalysisProps> = memo(
  ({ sleepData, period = 'week' }) => {
    ...
    const scoreStars = useMemo(() => {
      ...
      return [...Array(5)].map((_, i) => (
        <Star
          key={i}
          className={`h-6 w-6 ${
            i < Math.ceil(advice.score / 20) ? 'text-yellow-400 fill-current' : 'text-gray-300'
          }`}
        />
      ))
    }, [advice?.score])
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg p-4 mb-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center">{scoreStars}</div>
          </div>
        </div>
...
SleepAnalysis.displayName = 'SleepAnalysis'
