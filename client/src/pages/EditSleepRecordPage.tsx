...
const EditSleepRecordPage = () => {
  ...
  const fetchRecord = async (recordId: number) => {
    try {
      const record = await sleepService.getSleepRecord(recordId)
      setFormData({
        date: record.date,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await sleepService.updateSleepRecord(Number(id), formData)
      navigate(`/sleep/${id}`)
    } catch (error) {
      setError(error instanceof Error ? error.message : '기록 수정에 실패했습니다.')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">수면 기록 수정</h1>
        <p className="mt-1 text-sm text-gray-600">수면 정보를 수정하세요</p>
      </div>
...
export default EditSleepRecordPage
